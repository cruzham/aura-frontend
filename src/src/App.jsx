...
// --- LOCAL STORAGE KEY UTILITIES ---
const STORAGE_KEYS = {
  VIEW: 'aura_view',
  WORKSPACE: 'aura_workspace_name',
  INDUSTRY: 'aura_industry_focus',
  TIER: 'aura_current_tier',
  BILLING: 'aura_billing_period',
  PROMPT: 'aura_app_prompt',
  APP_NAME: 'aura_app_name_input',
  ACCENT: 'aura_selected_accent',
  FLEET: 'aura_fleet_data',
  AGENTS: 'aura_agents_data',
  INVOICES: 'aura_sandbox_invoices',
  DARK_MODE: 'aura_sandbox_dark_mode',
  ROOM_ID: 'aura_room_id',
  INSTALLED_MARKETPLACE: 'aura_installed_marketplace'
};
...
if (!user) return (
  ...
  const [view, setView] = useState(() => {
    return localStorage.getItem(STORAGE_KEYS.VIEW) || 'landing';
  });
  
  const [workspaceName, setWorkspaceName] = useState(() => {
    return localStorage.getItem(STORAGE_KEYS.WORKSPACE) || 'Stellar Labs';
  });

  const [industryFocus, setIndustryFocus] = useState(() => {
    return localStorage.getItem(STORAGE_KEYS.INDUSTRY) || 'saas';
  });

  const [currentTier, setCurrentTier] = useState(() => {
    return localStorage.getItem(STORAGE_KEYS.TIER) || 'Sandbox';
  });
  ...
  const [appPrompt, setAppPrompt] = useState(() => {
    return localStorage.getItem(STORAGE_KEYS.PROMPT) || 'A sleek client-portal and invoice tracker for freelance designers.';
  });

  const [appNameInput, setAppNameInput] = useState(() => {
    return localStorage.getItem(STORAGE_KEYS.APP_NAME) || 'DesignFlow CRM';
  });
  ...
  // ----------------------------------------------------
  // MARKETPLACE & CUSTOM INTEGRATION STATES
  // ----------------------------------------------------
  ...
  const marketplaceDatabase = useMemo(() => [
    {
      id: 'mp-deepseek',
      name: 'DeepSeek-Coder v3 Node',
      developer: 'DeepSeek Research',
      description: 'An advanced coding core tailored for intense logical backend synthesis and low-latency API compilation.',
      rating: '4.9',
      downloads: '12.4k',
      cost: 'Free tier / API Link',
      category: 'coding',
      capabilities: 'Wasm optimization, complex AST rendering'
    },
    {
      id: 'mp-stripe-arch',
      name: 'Stripe Checkout Architect',
      developer: 'Fintech Agent Lab',
      description: 'Dedicated checkout specialist that handles compliance setup, tax computations, and localized billing routing automatically.',
      rating: '4.8',
      downloads: '8.2k',
      cost: 'Included in Solo+',
      category: 'infrastructure',
      capabilities: 'SCA compliance, multi-currency ledger maps'
    },
    {
      id: 'mp-multilingual',
      name: 'SEO Multi-Lingual Translatrix',
      developer: 'Apex-Apex Corp',
      description: 'Auto-translates synthesized frontends into 24 global languages, optimizing edge routing tags for local search visibility.',
      rating: '4.7',
      downloads: '5.1k',
      cost: 'Included in Scale+',
      category: 'marketing',
      capabilities: 'Hreflang tagging, dynamic localization routing'
    },
    {
      id: 'mp-redteam',
      name: 'Sentinel Security Auditing Red-Team',
      developer: 'Sentinel Cyber',
      description: 'Runs high-concurrency fuzz testing and security breach simulations in Sandbox mode before final compile commits.',
      rating: '4.9',
      downloads: '14.1k',
      cost: 'Sovereign tier exclusive',
      category: 'infrastructure',
      capabilities: 'Breach detection, DDoS load balancing'
    }
  ], []);

  const filteredMarketplace = useMemo(() => {
    return marketplaceDatabase.filter(item => {
  ...
  const activeCodingAgentName = useMemo(() => {
    const hasDeepSeek = installedMarketplaceIds.includes('mp-deepseek');
    return hasDeepSeek ? 'DeepSeek-Coder v3 Node' : 'Cortex (Engineer)';
  }, [installedMarketplaceIds]);

  const activeCheckoutAgentName = useMemo(() => {
    const hasStripeArch = installedMarketplaceIds.includes('mp-stripe-arch');
    return hasStripeArch ? 'Stripe Checkout Architect' : 'Cortex (Engineer)';
  }, [installedMarketplaceIds]);

  const activeSecurityAgentName = useMemo(() => {
    const hasRedTeam = installedMarketplaceIds.includes('mp-redteam');
    return hasRedTeam ? 'Red-Team Security Auditor' : 'Sentinel (DevOps)';
  }, [installedMarketplaceIds]);
  ...
  // ----------------------------------------------------
  // INTEGRATED PRODUCTION STRIPE BILLING ENGINES
  // ----------------------------------------------------
  const handleUpgrade = async (tier) => {
    try {
      ...
      // Hand over secure routing flow to Stripe Hosted Gateway
      window.location.href = url;
    } catch (error) {
      console.error("Billing integration exception:", error);
      setIsUpgrading(null);
  ...
  const handleOpenBillingPortal = async () => {
    try {
      ...
      const response = await fetch('/api/create-portal-session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: "current_authenticated_user_id" })
      });
  ...
  // Real-time synchronization for deployed sandbox preview
  useEffect(() => {
    setDeployedAppName(appNameInput || "Aura Micro-App");
    setDeployedAccent(selectedAccent);
  }, [appNameInput, selectedAccent]);

  // ----------------------------------------------------
  // RUNTIME STATE PERSISTENCE SYNC
  // ----------------------------------------------------
  ...
  const handleCreateRoom = () => {
    ...
    setCoBuilders([
      { id: 'host', name: 'You (Owner)', status: 'Active', avatar: '👑' },
      { id: 'peer-1', name: 'DevNode_74b', status: 'Connecting...', avatar: '🤖' }
    ]);
    setMultiplayerLogs([
      `[System] Sync room ${randomCode} established.`,
      `[System] Node handshake verified. SSL Edge tunnel initialized.`
    ]);
  ...
  const handleJoinRoom = (e) => {
    ...
    setIsMultiplayerActive(true);
    setCoBuilders([
      { id: 'host', name: 'You (Guest)', status: 'Active', avatar: '👤' },
      { id: 'peer-1', name: 'MasterNode_01', status: 'Active', avatar: '👑' },
      { id: 'peer-2', name: 'Designer_Bot_x', status: 'Active', avatar: '🎨' }
    ]);
    setMultiplayerLogs([
      `[System] Handshake requested for room: ${cleanId}`,
      `[System] Connected. Received state payload bundle from MasterNode_01.`
    ]);
  ...
  const startMultiplayerSimulation = () => {
    ...
    mpSimulationRef.current = setInterval(() => {
      ...
      if (cycle === 1) {
        setCoBuilders(prev => prev.map(p => p.id === 'peer-1' ? { ...p, status: 'Active' } : p));
        setMultiplayerLogs(prev => [...prev, `[System] Peer DevNode_74b status updated to ACTIVE.`]);
      } else if (cycle === 3) {
        setMultiplayerLogs(prev => [...prev, `[DevNode_74b] Modified requirement prompt: Added localized translation tags.`]);
        setAppPrompt(prev => prev + " Inject language support components.");
      } else if (cycle === 5) {
        setMultiplayerLogs(prev => [...prev, `[DevNode_74b] Recalibrated Senior Dev Swarm creative temperature to 0.4.`]);
        setAgents(prev => prev.map(a => a.id === 'dev' ? { ...a, temperature: 0.4 } : a));
      } else if (cycle === 7) {
        setMultiplayerLogs(prev => [...prev, `[System] Keep-alive packet verified. Sync active.`]);
      }
    }, 4500);
  };
  ...
  const toggleAgentInstallation = (id, name) => {
    ...
    if (isInstalled) {
      setInstalledMarketplaceIds(prev => prev.filter(item => item !== id));
      setBanner(`De-registered ${name} from core stack.`);
    } else {
      setInstalledMarketplaceIds(prev => [...prev, id]);
      setBanner(`Successfully integrated ${name} into available neural lanes.`);
    }
  };
  ...
  const runBuildProcess = () => {
    ...
    setBuildStep(1);
    setBuildProgress(5);
    setActiveWorkingAgent('Product Manager (Atlas)');
    ...
    intervalRef.current = setInterval(() => {
      if (logIndex < logs.length) {
        ...
        setBuildStep(logIndex + 1);
        setBuildProgress(nextLog.progress);
        setActiveWorkingAgent(nextLog.agent);
        setBuildLogs(prev => [...prev, nextLog.text]);
        logIndex++;
      } else {
        clearInterval(intervalRef.current);
        setIsBuilding(false);
        setActiveWorkingAgent('');
        ...
        const newApp = {
          id: `app-${Date.now()}`,
          name: appNameInput || "Aura Micro-App",
          status: 'Active',
          mrr: Math.floor(Math.random() * 850) + 300,
  ...
  const addInvoice = (e) => {
    ...
    const newInv = {
      id: `INV-00${sandboxInvoices.length + 1}`,
      client: newClient,
      amount: parseFloat(newAmount),
      status: 'Pending',
      date: 'Today'
    };
    setSandboxInvoices(prev => [newInv, ...prev]);
    setNewClient('');
    setNewAmount('');
  };
  ...
  const exportSystemBundle = () => {
    const backupPackage = {
      ...
      aura_os_version: '2.5.8-UltimateMarketplace',
      payload: {
        workspaceName,
        industryFocus,
        currentTier,
  ...
  const clearSystemDatabase = () => {
    if (window.confirm("Warning: This action completely flashes your local storage clusters. Reset platform configuration?")) {
      localStorage.clear();
  ...
  return (
    <div className="min-h-screen bg-[#06060a] text-slate-200 font-sans selection:bg-indigo-500/30">
      ...
      {banner && (
        <div className="bg-gradient-to-r from-indigo-950 via-slate-900 to-slate-950 border-b border-indigo-500/20 text-xs py-3 px-4 relative z-50 flex items-center justify-between">
          <div className="flex items-center gap-2 mx-auto">
            ...
            <span className="font-mono text-slate-300">{banner}</span>
          </div>
          <button onClick={() => setBanner('')} className="text-slate-400 hover:text-white">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigateTo('landing')}>
              ...
              <div className="flex flex-col">
                <span className="text-lg font-bold tracking-tight text-white leading-none">AURA</span>
                <span className="text-[10px] text-slate-500 tracking-wider font-semibold">ECOSYSTEM HUB INSTANTIATED</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              {view !== 'landing' && (
                <button 
                  onClick={exportSystemBundle}
                  title="Export System Bundle"
                  className="p-2 bg-white/[0.03] border border-white/10 text-slate-400 hover:text-white rounded-lg transition-colors flex items-center gap-1.5 text-xs font-semibold"
                >
                  <Download className="w-4 h-4" /> <span className="hidden sm:inline">Export Bundle</span>
                </button>
              )}
                ...
                <div className="flex items-center gap-2">
                  {currentTier !== 'Sandbox' && (
                    <button 
                      ...
                      className="hidden sm:flex px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-slate-400 hover:text-slate-200 rounded-lg text-[11px] font-bold tracking-tight items-center gap-1"
                    >
                      <SlidersHorizontal className="w-3.5 h-3.5" /> Manage Invoices
                    </button>
                  )}
        ...
        <div className="relative pt-24 pb-32 overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 text-center relative z-10">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-semibold mb-8">
              <Zap className="w-3.5 h-3.5" /> <span>LOCAL MEMORY & INTEGRATED PLUGINS SYSTEM ACTIVE</span>
            </div>
            <h1 className="text-5xl md:text-8xl font-black tracking-tight text-white mb-6">
              Empose an Empire with<br/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400">Custom Neural Swarms.</span>
            </h1>
            <p className="max-w-2xl text-base md:text-lg text-slate-400 mx-auto mb-10">
              Aura doesn't restrict you to baseline pipelines. Bring fine-tuned LLMs, community specialized PMs, and enterprise auditing agents directly into your persistent compilation loop.
            </p>
        ...
        <div className="max-w-xl mx-auto px-4 py-20 relative z-10">
          <div className="bg-[#0f0f16] border border-white/10 rounded-3xl p-8 shadow-2xl">
            ...
            <form onSubmit={(e) => { e.preventDefault(); navigateTo('dashboard'); }} className="space-y-6">
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-2">Cluster Organization Identifier</label>
                <input type="text" value={workspaceName} onChange={(e) => setWorkspaceName(e.target.value)} className="w-full bg-white/[0.03] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-indigo-500 text-sm" required />
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button type="button" onClick={() => setIndustryFocus('saas')} className={`p-4 rounded-xl border text-left transition-all ${industryFocus === 'saas' ? 'border-indigo-500 bg-indigo-500/10' : 'border-white/5 bg-white/[0.02]'}`}>
                    <FolderKanban className="w-5 h-5 text-indigo-400 mb-2" />
                    <p className="font-bold text-xs text-white">Saas Core Hub</p>
                  </button>
                  <button type="button" onClick={() => setIndustryFocus('commerce')} className={`p-4 rounded-xl border text-left transition-all ${industryFocus === 'commerce' ? 'border-indigo-500 bg-indigo-500/10' : 'border-white/5 bg-white/[0.02]'}`}>
                    <DollarSign className="w-5 h-5 text-purple-400 mb-2" />
                    <p className="font-bold text-xs text-white">Billing Gateways</p>
                  </button>
                </div>
              </div>
      ...
      {/* ================= VIEW 3: OPERATIONS FLEET ================= */}
      {view === 'dashboard' && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
          <div className="flex justify-between items-center mb-10">
            <div>
              <h2 className="text-2xl font-black text-white">Active Fleet Systems</h2>
              <p className="text-slate-400 text-xs">Persistent configuration records running inside safe browser disk structures.</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
            <div className="p-5 rounded-xl bg-white/[0.02] border border-white/5">
              <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Portfolio MRR Valuation</span>
              <p className="text-2xl font-bold text-white">${fleet.reduce((acc, curr) => acc + curr.mrr, 0).toLocaleString()}</p>
            </div>
            <div className="p-5 rounded-xl bg-white/[0.02] border border-white/5">
              <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Hydrated Active End-Users</span>
              <p className="text-2xl font-bold text-indigo-400">{fleet.reduce((acc, curr) => acc + curr.users, 0).toLocaleString()}</p>
            </div>
            <div className="p-5 rounded-xl bg-white/[0.02] border border-white/5">
              <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Ecosystem Plugins Synced</span>
              <p className="text-2xl font-bold text-green-400">{installedMarketplaceIds.length} Nodes Active</p>
            </div>
          </div>
            ...
            {fleet.map((app) => {
              ...
              return (
                <div key={app.id} className="p-5 bg-[#0f0f16] border border-white/5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-white/[0.02] flex items-center justify-center border border-white/5">
                      <Code className="w-4 h-4" style={{ color: appStyles.fill }} />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-bold text-white text-sm">{app.name}</h4>
                        <span className={`text-[9px] font-extrabold px-1.5 rounded ${appStyles.badge}`}>{app.status}</span>
                      </div>
                      <p className="text-[11px] text-slate-400 mt-0.5">{app.prompt}</p>
                    </div>
                  </div>
      ...
      {/* ================= VIEW 4: COMPILER ASSEMBLY & SIMULATOR ================= */}
      {view === 'workspace' && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
          
          {/* Multiplayer Room Controller */}
          <div className="mb-8 p-6 bg-[#0f0f16] border border-white/10 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
            ...
            <div className="flex items-center gap-4 relative z-10">
              ...
              <div>
                <h3 className="font-extrabold text-sm text-white">Multiplayer Node Sync</h3>
                <p className="text-[11px] text-slate-400 mt-0.5 font-medium">Invite co-builders or simulate team interaction pipelines over our simulated sockets.</p>
              </div>
            </div>
              ...
              {!isMultiplayerActive ? (
                <>
                  <button 
                    ...
                    className="flex-1 md:flex-initial px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-xs font-bold text-white rounded-lg transition-colors flex items-center justify-center gap-1.5"
                  >
                    <Plus className="w-3.5 h-3.5" /> Start Synced Session
                  </button>
                  <form onSubmit={handleJoinRoom} className="flex gap-2 w-full sm:w-auto">
                    <input 
                      ...
                      className="bg-white/[0.02] border border-white/10 rounded-lg px-3 py-2 text-xs text-white focus:border-indigo-500 outline-none w-full sm:w-32"
                    />
                    <button 
                      type="submit"
                      className="px-4 py-2 bg-white/5 border border-white/10 text-xs font-semibold rounded-lg hover:bg-white/10 transition-colors text-slate-300"
                    >
                      Join
                    </button>
                  </form>
                ...
                <div className="flex items-center gap-4 bg-black/40 border border-white/5 rounded-xl px-4 py-2 w-full md:w-auto justify-between">
                    ...
                    <div className="flex items-center -space-x-2">
                      {coBuilders.map(p => (
                        <div 
                          ...
                          className="w-7 h-7 rounded-full bg-slate-800 border-2 border-[#0f0f16] flex items-center justify-center text-xs relative cursor-pointer"
                        >
                          {p.avatar}
                          <span className={`absolute bottom-0 right-0 w-2 h-2 rounded-full border border-[#0f0f16] ${p.status === 'Active' ? 'bg-green-500' : 'bg-yellow-500 animate-pulse'}`}></span>
                        </div>
                      ))}
                    </div>
                  </div>
                  <button 
                    onClick={leaveRoom}
                    className="text-xs text-red-400 hover:text-red-300 font-semibold border-l border-white/10 pl-3.5"
                  >
                    Leave
                  </button>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-4 bg-[#0f0f16] border border-white/10 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                ...
                {isMultiplayerActive && (
                  <span className="text-[10px] text-indigo-400 font-mono font-bold flex items-center gap-1 animate-pulse">
                    <RadioTower className="w-3 h-3" /> SYNCED
                  </span>
                )}
              </div>
              <div>
                <label className="block text-[10px] text-slate-500 mb-1">Micro-SaaS Core Label</label>
                <input type="text" value={appNameInput} onChange={(e) => setAppNameInput(e.target.value)} className="w-full bg-white/[0.02] border border-white/10 rounded-lg px-3 py-2 text-xs text-white focus:border-indigo-500 outline-none" />
              </div>
              <div>
                <label className="block text-[10px] text-slate-500 mb-1">UI Interface Accents</label>
                <div className="flex gap-2">
                  {['indigo', 'emerald', 'rose', 'amber'].map(c => (
                    <button key={c} onClick={() => setSelectedAccent(c)} className={`w-5 h-5 rounded-full border transition-transform ${selectedAccent === c ? 'scale-110 border-white' : 'border-transparent'}`} style={{ backgroundColor: c === 'indigo' ? '#6366f1' : c === 'emerald' ? '#10b981' : c === 'rose' ? '#f43f5e' : '#f59e0b' }} />
                  ))}
                </div>
              </div>
              <div>
                <label className="block text-[10px] text-slate-500 mb-1">Functional Architecture Requirement</label>
                <textarea value={appPrompt} onChange={(e) => setAppPrompt(e.target.value)} className="w-full h-24 bg-white/[0.02] border border-white/10 rounded-lg p-2.5 text-xs text-slate-300 focus:border-indigo-500 outline-none resize-none" />
              </div>
              
              {installedMarketplaceIds.length > 0 && (
                <div className="p-3.5 rounded-xl bg-indigo-500/5 border border-indigo-500/20 text-[10px] text-indigo-300 flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-indigo-400 flex-shrink-0" />
                  <div>
                    <span className="font-bold">Ecosystem Override:</span> Compiling using: <span className="font-semibold text-white">{activeCodingAgentName}</span>.
                  </div>
                </div>
              )}

              <button onClick={runBuildProcess} disabled={isBuilding} className={`w-full py-3 text-xs font-bold rounded-xl text-white ${isBuilding ? 'bg-indigo-600/20 text-white/40 cursor-not-allowed' : `${activePromptStyles.bg} hover:opacity-90`}`}>
                {isBuilding ? 'Compiling Assembly Network...' : 'Commit Code Pipeline to Disk'}
              </button>
            </div>

            {/* Simulated Live Workspace Output */}
            <div className="lg:col-span-8 space-y-4">
              
              {isMultiplayerActive && (
                <div className="bg-[#050508] border border-indigo-500/20 rounded-xl p-4 font-mono text-xs">
                  <p className="text-indigo-400 font-bold mb-2 flex items-center gap-1.5 text-[10px] uppercase tracking-wider">
                    <MessageSquare className="w-3.5 h-3.5 animate-pulse" /> Peer-to-Peer Data Events
                  </p>
                  <div className="space-y-1 text-slate-400 text-[10px] max-h-[80px] overflow-y-auto">
                    {multiplayerLogs.map((log, idx) => (
                      <p key={idx} className={log.startsWith('[System]') ? 'text-indigo-400/80' : 'text-slate-300'}>
                        {log}
                      </p>
                    ))}
                  </div>
              ...
              <div className="bg-[#050508] border border-white/10 rounded-xl p-4 font-mono text-xs">
                ...
                <div className="w-full bg-white/5 h-1 rounded-full overflow-hidden mb-3">
                  <div className="h-full bg-indigo-500 transition-all duration-300" style={{ width: `${buildProgress}%` }}></div>
                </div>
      ...
      {/* ================= VIEW 5: WORKFORCE PARAMETERS ================= */}
      {view === 'agents' && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
          <div className="mb-6">
            <h2 className="text-2xl font-black text-white">Agent Thread Matrix</h2>
            <p className="text-slate-400 text-xs">Calibrate token budgets. Changes commit to local database engines instantly.</p>
          </div>
              ...
              <div key={ag.id} className="p-5 bg-[#0f0f16] border border-white/5 rounded-2xl flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-center border-b border-white/5 pb-3 mb-4">
                    <div><h4 className="font-bold text-white text-sm">{ag.name}</h4><span className="text-[11px] text-slate-500">{ag.role}</span></div>
                  </div>
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between text-[11px] mb-1"><span className="text-slate-400">Model Temperature Vector</span><span className="text-white font-mono">{ag.temperature}</span></div>
                      <input type="range" min="0" max="1" step="0.1" value={ag.temperature} onChange={(e) => { const val = parseFloat(e.target.value); setAgents(agents.map(a => a.id === ag.id ? { ...a, temperature: val } : a)); }} className="w-full accent-indigo-500 h-1 bg-white/5 rounded" />
                    </div>
                    <div>
                      <div className="flex justify-between text-[11px] mb-1"><span className="text-slate-400">Compute Allocation Cap</span><span className="text-white font-mono">${ag.budget}</span></div>
                      <input type="range" min="50" max="2000" step="50" value={ag.budget} onChange={(e) => { const val = parseInt(e.target.value); setAgents(agents.map(a => a.id === ag.id ? { ...a, budget: val } : a)); }} className="w-full accent-indigo-500 h-1 bg-white/5 rounded" />
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {['Sandbox', 'Solo', 'Scale'].map((t) => (
              <div key={t} className={`p-5 rounded-2xl bg-[#0f0f16] border flex flex-col justify-between ${currentTier === t ? 'border-indigo-500 shadow-lg' : 'border-white/5'}`}>
                <div>
                  <h3 className="font-extrabold text-white text-base">{t} Node Cluster</h3>
                  <p className="text-xs text-slate-400 my-3 leading-relaxed">Expand simulation framework token targets and scale computational limits inside database loops.</p>
                </div>
      ...
      {/* ================= VIEW 7: AGENT MARKETPLACE & CREDENTIALS VAULT ================= */}
      {view === 'marketplace' && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
          
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 pb-6 border-b border-white/5">
            <div>
              <h2 className="text-2xl font-black text-white">Agent Ecosystem & Plugins</h2>
              <p className="text-slate-400 text-xs">Integrate third-party neural modules to perform specialized design and security pipelines.</p>
            </div>

            <div className="flex items-center gap-2 bg-white/[0.02] p-1 rounded-xl border border-white/5 self-start md:self-auto">
              {['all', 'coding', 'marketing', 'infrastructure'].map((cat) => (
                <button
                  key={cat}
                  onClick={() => setMarketplaceFilter(cat)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold capitalize transition-colors ${marketplaceFilter === cat ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                >
                  {cat}
                </button>
              ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-10 items-start">
            <div className="lg:col-span-8">
              <div className="relative mb-6">
                ...
                <input 
                  type="text"
                  placeholder="Search custom agents (e.g., DeepSeek, Compliance)..."
                  value={marketplaceSearch}
                  onChange={(e) => setMarketplaceSearch(e.target.value)}
                  className="w-full bg-[#0f0f16] border border-white/10 rounded-2xl pl-10 pr-4 py-3.5 text-xs text-white outline-none focus:border-indigo-500 transition-colors"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {filteredMarketplace.map((item) => {
                  ...
                  return (
                    <div 
                      key={item.id} 
                      className={`p-5 rounded-2xl bg-[#0f0f16] border transition-all ${
                        isInstalled ? 'border-indigo-500 bg-indigo-500/[0.01]' : 'border-white/5'
                      }`}
                    >
                      <div className="flex justify-between items-start gap-3 mb-3">
                        <div>
                          <h4 className="font-bold text-white text-sm">{item.name}</h4>
                          <span className="text-[10px] text-slate-500">By {item.developer}</span>
                        </div>
                        <span className="bg-white/5 text-[9px] text-slate-300 px-2 py-0.5 rounded font-mono font-semibold uppercase">{item.category}</span>
                      </div>

                      <p className="text-xs text-slate-400 leading-relaxed mb-4 min-h-[48px]">{item.description}</p>

                      <div className="p-3 bg-black/40 rounded-xl border border-white/5 mb-4 text-[10px] font-mono text-slate-500">
                        <span className="font-semibold text-slate-300 block mb-1">CAPABILITY VECTOR:</span>
                        {item.capabilities}
                      </div>
                      ...
                      <div className="flex justify-between items-center text-xs mt-6 pt-4 border-t border-white/5">
                        ...
                        <button
                          onClick={() => toggleAgentInstallation(item.id, item.name)}
                        ...
                        >
                          {isInstalled ? 'De-register Node' : 'Integrate Node'}
                        </button>
                      </div>
                    </div>
                  );
            ...
            <div className="lg:col-span-4 bg-[#0f0f16] border border-white/10 rounded-2xl p-5 space-y-4">
              <div className="flex items-center gap-2 mb-2 pb-2 border-b border-white/5">
                ...
                <h3 className="text-xs font-bold uppercase text-slate-300 tracking-wide">Credentials Vault</h3>
              </div>
              <p className="text-[11px] text-slate-400 leading-relaxed">
                Plug in custom keys for live API models. If left blank, simulation nodes run inside local neural sandboxes using mock variables.
              </p>

              <div className="space-y-3 pt-2">
                <div>
                  <label className="block text-[10px] text-slate-500 mb-1 font-mono uppercase">DeepSeek API Key Override</label>
                  <input 
                    type="password" 
                    placeholder="sk-ds-••••••••••••••••" 
                    className="w-full bg-white/[0.02] border border-white/10 rounded-lg px-3 py-2 text-xs text-slate-400 outline-none focus:border-indigo-500 font-mono" 
                  />
                </div>
                <div>
                  <label className="block text-[10px] text-slate-500 mb-1 font-mono uppercase">Stripe Production Secret Key</label>
                  <input 
                    type="password" 
              ...
              <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5 text-[10px] text-slate-500 leading-relaxed">
                <span className="font-bold text-slate-300 block mb-0.5">TLS Certificate Active</span>
                Credentials are encrypted under AES-256 blocks before local synchronization. Aura never handles plain-text credentials over centralized telemetry routers.
              </div>
            </div>
          </div>
      ...
      <footer className="border-t border-white/5 py-8 mt-20 bg-[#0a0a0e] text-center text-xs text-slate-600">
        © 2026 Aura Systems. Fully Persisted Local Sandbox Runtime.
      </footer>
    </div>
  );
}
